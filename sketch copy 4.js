...
void main() {
    ...
    float distFromTarget = abs((gl_FragCoord.y / u_resolution.y) - u_targetY);
    
    vec2 cloudSt = st * 3.0;
    cloudSt.x -= u_time * 0.15;
    
    vec2 q = vec2(0.0);
    q.x = fbm(cloudSt + vec2(0.0, u_time * 0.02));
    q.y = fbm(cloudSt + vec2(5.2, 1.3));
    
    float f = fbm(cloudSt + q * 2.0);
    
    float verticalMask = smoothstep(0.3, 0.0, distFromTarget);
    
    float cloudDensity = smoothstep(0.3, 0.7, f) * verticalMask;
    cloudDensity = pow(cloudDensity, 1.5);

    gl_FragColor = vec4(1.0, 1.0, 1.0, cloudDensity * 0.9);
}
`;

function createSky() {
    if (skyBuffer) skyBuffer.remove();
    skyBuffer = createGraphics(windowWidth, windowHeight);
    let c1 = color('#4A90E2');
    let c2 = color('#D1E8FF');
    for(let y = 0; y < windowHeight; y++) {
        let n = map(y, 0, windowHeight, 0, 1);
        let newc = lerpColor(c1, c2, n);
        skyBuffer.stroke(newc);
        skyBuffer.line(0, y, windowWidth, y);
    }
}
...
function setup() {
    ...
    // Create shader buffer for clouds
    cloudBuffer = createGraphics(windowWidth, windowHeight, WEBGL);
    cloudShader = cloudBuffer.createShader(vertShader, fragShader);
...
function draw() {
    if (skyBuffer) {
        image(skyBuffer, 0, 0); // Linear Gradient Background
    } else {
        background(210, 230, 240); // Fallback
    }
...
function drawTerrain() {
    ...
    let islandTopY = height * 0.6; 
    let islandHeight = height / 3; // INCREASED AMPLITUDE per user request (was height/5)
    ...
    // ==========================================
    // PROCEDURAL CLOUDS (fBm Shader)
    // ==========================================
    if (cloudBuffer && cloudShader) {
        ...
        // Target Y is the bottom tip of the island
        // WebGL origin is bottom-left, so we invert Y
        let targetY = 1.0 - ((islandTopY + islandHeight * 0.9) / height);
        cloudShader.setUniform('u_targetY', targetY);
        
        // Render shader to buffer
        cloudBuffer.noStroke();
        cloudBuffer.rect(-width/2, -height/2, width, height); 
        
        // Draw the cloud layer so it weaves between the back rock and front rock
...
function windowResized() {
    ...
    createSky();
    if (cloudBuffer) {
        cloudBuffer.resizeCanvas(windowWidth, windowHeight);
    }
